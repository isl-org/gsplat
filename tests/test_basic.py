...
requires_backend = pytest.mark.skipif(gsplat.BACKEND == "", reason="No CUDA or SYCL backend available")
requires_cuda = pytest.mark.skipif(gsplat.BACKEND != "cuda", reason="Test requires CUDA backend")
...
def test_quat_scale_to_covar_preci(test_data, triu: bool, batch_dims: Tuple[int, ...]):
    ...
    v_quats, v_scales = torch.autograd.grad((covars * v_covars + precis * v_precis).sum(), (quats, scales))
    _v_quats, _v_scales = torch.autograd.grad((_covars * v_covars + _precis * v_precis).sum(), (quats, scales))
...
def test_proj(test_data, camera_model: str, batch_dims: Tuple[int, ...]):
    ...
    from gsplat._torch_impl import (_fisheye_proj, _ortho_proj, _persp_proj, _world_to_cam)

    torch.manual_seed(42)
    test_data = expand(test_data, batch_dims)
    Ks, viewmats, height, width = test_data["Ks"], test_data["viewmats"], test_data["height"], test_data["width"]

    covars, _ = gsplat.quat_scale_to_covar_preci(test_data["quats"], test_data["scales"])
    ...
    if camera_model == "ortho": _means2d, _covars2d = _ortho_proj(means, covars, Ks, width, height)
    elif camera_model == "fisheye": _means2d, _covars2d = _fisheye_proj(means, covars, Ks, width, height)
    elif camera_model == "pinhole": _means2d, _covars2d = _persp_proj(means, covars, Ks, width, height)
    else: assert_never(camera_model)
    ...
    v_means, v_covars = torch.autograd.grad((means2d * v_means2d).sum() + (covars2d * v_covars2d).sum(), (means, covars))
    _v_means, _v_covars = torch.autograd.grad((_means2d * v_means2d).sum() + (_covars2d * v_covars2d).sum(), (means, covars))
...
@pytest.mark.parametrize("batch_dims", [(), (2,), (1, 2)])
def test_projection(test_data, fused: bool, calc_compensations: bool, camera_model: str, batch_dims: Tuple[int, ...]):
    ...
    v_means2d, v_depths, v_conics = torch.randn_like(means2d) * valid[..., None], torch.randn_like(depths) * valid, torch.randn_like(conics) * valid[..., None]
    v_compensations = torch.randn_like(compensations) * valid if calc_compensations else 0
    grad_sum = (means2d * v_means2d).sum() + (depths * v_depths).sum() + (conics * v_conics).sum() + ((compensations * v_compensations).sum() if calc_compensations else 0)
    v_viewmats, v_quats, v_scales, v_means = torch.autograd.grad(grad_sum, (viewmats, quats, scales, means))
    
    _grad_sum = (_means2d * v_means2d).sum() + (_depths * v_depths).sum() + (_conics * v_conics).sum() + ((_compensations * v_compensations).sum() if calc_compensations else 0)
    _v_viewmats, _v_quats, _v_scales, _v_means = torch.autograd.grad(_grad_sum, (viewmats, quats, scales, means))
...
@pytest.mark.parametrize("camera_model", ["pinhole", "ortho", "fisheye"])
@pytest.mark.parametrize("batch_dims", [(), (2,), (1, 2)])
def test_fully_fused_projection_packed(test_data, fused: bool, sparse_grad: bool, calc_compensations: bool, camera_model: str, batch_dims: Tuple[int, ...]):
    ...
    if fused:
        res = gsplat.fully_fused_projection(
            means, None, quats, scales, viewmats, Ks, width, height, packed=True,
            sparse_grad=sparse_grad, calc_compensations=calc_compensations, camera_model=camera_model
        )
        _radii, _means2d, _depths, _conics, _compensations = gsplat.fully_fused_projection(
        ...
        covars, _ = gsplat.quat_scale_to_covar_preci(quats, scales, triu=True)
        res = gsplat.fully_fused_projection(
            means, covars, None, None, viewmats, Ks, width, height, packed=True,
            sparse_grad=sparse_grad, calc_compensations=calc_compensations, camera_model=camera_model
        )
        _radii, _means2d, _depths, _conics, _compensations = gsplat.fully_fused_projection(
    ...
    batch_ids, camera_ids, gaussian_ids, radii, means2d, depths, conics, compensations = res
    ...
    __radii = torch.sparse_coo_tensor(indices, radii, sparse_shape + (2,)).to_dense().reshape(batch_dims + (C, N, 2))
    __means2d = torch.sparse_coo_tensor(indices, means2d, sparse_shape + (2,)).to_dense().reshape(batch_dims + (C, N, 2))
    __depths = torch.sparse_coo_tensor(indices, depths, sparse_shape).to_dense().reshape(batch_dims + (C, N))
    __conics = torch.sparse_coo_tensor(indices, conics, sparse_shape + (3,)).to_dense().reshape(batch_dims + (C, N, 3))
    if calc_compensations:
        __compensations = torch.sparse_coo_tensor(indices, compensations, sparse_shape).to_dense().reshape(batch_dims + (C, N))
    ...
    v_means2d, v_depths, v_conics = torch.randn_like(_means2d) * sel[..., None], torch.randn_like(_depths) * sel, torch.randn_like(_conics) * sel[..., None]
    _v_viewmats, _v_quats, _v_scales, _v_means = torch.autograd.grad(
        (_means2d * v_means2d).sum() + (_depths * v_depths).sum() + (_conics * v_conics).sum(),
        (viewmats, quats, scales, means), retain_graph=True
    )
    v_viewmats, v_quats, v_scales, v_means = torch.autograd.grad(
        (means2d * v_means2d[sel]).sum() + (depths * v_depths[sel]).sum() + (conics * v_conics[sel]).sum(),
        (viewmats, quats, scales, means), retain_graph=True
    )
    if sparse_grad:
        v_quats, v_scales, v_means = v_quats.to_dense(), v_scales.to_dense(), v_means.to_dense()
...
def test_isect(test_data, batch_dims: Tuple[int, ...]):
    ...
    test_data = expand(test_data, batch_dims)
    means2d, radii, depths = test_data["means2d"], test_data["radii"], test_data["depths"]

    tile_size = 16
    tile_width, tile_height = math.ceil(width / tile_size), math.ceil(height / tile_size)

    tiles_per_gauss, isect_ids, flatten_ids = gsplat.isect_tiles(means2d, radii, depths, tile_size, tile_width, tile_height)
...
def test_rasterize_to_pixels(test_data, channels: int, batch_dims: Tuple[int, ...]):
    ...
    test_data = expand(test_data, batch_dims)
    Ks, viewmats, height, width = test_data["Ks"], test_data["viewmats"], test_data["height"], test_data["width"]
    quats, scales, means, opacities = test_data["quats"], test_data["scales"] * 0.1, test_data["means"], test_data["opacities"]
    colors, backgrounds = test_data["colors"], test_data["backgrounds"]

    covars, _ = gsplat.quat_scale_to_covar_preci(quats, scales, compute_preci=False, triu=True)
    radii, means2d, depths, conics, _ = gsplat.fully_fused_projection(means, covars, None, None, viewmats, Ks, width, height)
    opacities = torch.broadcast_to(opacities[..., None, :], batch_dims + (C, N))

    tile_size = 16 if channels <= 32 else 4
    tile_width, tile_height = math.ceil(width / float(tile_size)), math.ceil(height / float(tile_size))
    tiles_per_gauss, isect_ids, flatten_ids = gsplat.isect_tiles(means2d, radii, depths, tile_size, tile_width, tile_height)
    isect_offsets = gsplat.isect_offset_encode(isect_ids, I, tile_width, tile_height).reshape(batch_dims + (C, tile_height, tile_width))
    ...
    render_colors, render_alphas = gsplat.rasterize_to_pixels(
        ...
        isect_offsets, flatten_ids, backgrounds=backgrounds
    )
    ...
    v_render_colors, v_render_alphas = torch.randn_like(render_colors), torch.randn_like(render_alphas)
