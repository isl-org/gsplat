...
void launch_rasterize_kernel(
    ...
    auto e = d_queue.submit(
        [&](sycl::handler& cgh)
        {
            ...
            sycl::local_accessor<sycl::vec<float, 2>, 1> slm_means2d(slm_range, cgh);
            ...
            sycl::local_accessor<BufferType_t<float, COLOR_DIM>, 1> slm_color;
             if constexpr(BufferType<float, COLOR_DIM>::isVec && COLOR_DIM <= 4) {
                slm_color = sycl::local_accessor<BufferType_t<float, COLOR_DIM>, 1>(slm_range, cgh);
             }

            RasterizeToPixelsFwdKernel<COLOR_DIM, CHUNK_SIZE, float, false> kernel(
                C, N, flatten_ids.size(0), packed,
...
#define __GS__CALL_(DIM)                                                              \
    case DIM:                                                                         \
        launch_rasterize_kernel<DIM>(                                                 \
            ...
            backgrounds, masks, image_width, image_height, tile_size, tile_offsets,   \
            flatten_ids, packed, C, N, tile_height, tile_width,                       \
