...
at::Tensor spherical_harmonics_fwd(
    ...
    if (masks.has_value()) {
        TORCH_CHECK(masks.value().is_contiguous(), "Input 'masks' tensor must be contiguous.");
    }
    
    TORCH_CHECK(dirs.size(-1) == 3, "Input 'dirs' tensor must have the last dimension of size 3.");
    TORCH_CHECK(coeffs.size(-1) == 3, "Input 'coeffs' tensor must have the last dimension of size 3.");
