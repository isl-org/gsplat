...
std::tuple<at::Tensor, at::Tensor, at::Tensor> intersect_tile(
    ...
    if (image_ids.has_value()) CHECK_CONTIGUOUS(image_ids.value());
    if (gaussian_ids.has_value()) CHECK_CONTIGUOUS(gaussian_ids.value());
    ...
    TORCH_CHECK(tile_n_bits + cam_n_bits <= 32, "Not enough bits to encode camera and tile IDs.");

    auto& d_queue = at::xpu::getCurrentXPUStream().queue();
    size_t numWorkGrps = (total_elems + GSPLAT_N_THREADS - 1) / GSPLAT_N_THREADS;
    ...
    at::Tensor cum_tiles_per_gauss = at::cumsum(tiles_per_gauss.view({-1}), 0, at::kLong);
    int64_t n_isects = 0;
    if (total_elems > 0) {
