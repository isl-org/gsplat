...
void launch_rasterize_2dgs_bwd_kernel(
    ...
    auto e = d_queue.submit(
        ...
        {            
            ...
            sycl::local_accessor<BufferType_t<float, COLOR_DIM>, 1> slm_rgbs(chunk_size, cgh);
            sycl::local_accessor<sycl::vec<float, 3>, 1> slm_normals(chunk_size, cgh);
            ...
            RasterizeToPixels2DGSBwdKernel<COLOR_DIM> kernel(
                ...
                v_means2d_abs.has_value() ? 
                    reinterpret_cast<sycl::vec<float, 2>*>(v_means2d_abs.value().data_ptr<float>()) : nullptr,
                ...
                slm_id_batch, slm_xy_opacity, slm_u_Ms, slm_v_Ms, slm_w_Ms, slm_rgbs, slm_normals
            );
