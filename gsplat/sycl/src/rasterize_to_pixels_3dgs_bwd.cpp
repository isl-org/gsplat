...
void launch_rasterize_bwd_kernel(
    ...
    auto e = d_queue.submit([&](sycl::handler &cgh) {
        ...
        if constexpr (BufferType<float, COLOR_DIM>::isVec && COLOR_DIM <= 4) {
            slm_color =
                sycl::local_accessor<BufferType_t<sycl::half, COLOR_DIM>, 1>(
                    slm_range, cgh
                );
        }
        ...
        RasterizeToPixelsBwdKernel<COLOR_DIM, CHUNK_SIZE, float, false> kernel(
            ...
            absgrad ? reinterpret_cast<sycl::vec<float, 2> *>(
...
rasterize_to_pixels_3dgs_bwd(
    ...
    // gradients of outputs
    const at::Tensor
        v_render_colors, // [..., C, image_height, image_width, COLOR_DIM]
    ...
    at::Tensor v_means2d_abs =
        absgrad ? at::zeros_like(means2d) : at::empty({0}, means2d.options());
