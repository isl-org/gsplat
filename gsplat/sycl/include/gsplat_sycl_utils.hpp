...
template <typename S, uint32_t N> struct BufferType {
    using type = sycl::marray<S, N>;
    constexpr static bool isVec{false};
};

template <typename S> struct BufferType<S, 2> {
    using type = sycl::vec<S, 2>;
    constexpr static bool isVec{true};
};

template <typename S> struct BufferType<S, 3> {
    using type = sycl::vec<S, 3>;
    constexpr static bool isVec{true};
};

template <typename S> struct BufferType<S, 4> {
    using type = sycl::vec<S, 4>;
    constexpr static bool isVec{true};
};

template <typename S> struct BufferType<S, 8> {
    using type = sycl::vec<S, 8>;
    constexpr static bool isVec{true};
};

template <typename S> struct BufferType<S, 16> {
...
template <typename T> void readToBuffer(T &dest, const void *source) {
...
template <typename T> void gpuAtomicAddGlobal(T &ref, const T &value) {
    sycl::atomic_ref<
        T,
        sycl::memory_order::relaxed,
        sycl::memory_scope::device,
...
template <typename T> void gpuAtomicAddLocal(T &ref, const T &value) {
    sycl::atomic_ref<
        T,
        sycl::memory_order::relaxed,
        sycl::memory_scope::device,
