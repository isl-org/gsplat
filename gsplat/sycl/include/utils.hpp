...
template <typename T>
inline void inverse_vjp(const T Minv, const T v_Minv, T &v_M) {
    // P = M^-1
    // df/dM = -P * df/dP * P
...
template <typename T>
inline T add_blur(const T eps2d, mat2<T> &covar, T &compensation) {
    T det_orig = covar[0][0] * covar[1][1] - covar[0][1] * covar[1][0];
    covar[0][0] += eps2d;
    covar[1][1] += eps2d;
    T det_blur = covar[0][0] * covar[1][1] - covar[0][1] * covar[1][0];
    compensation =
        sycl::sqrt(sycl::max(static_cast<T>(0), det_orig / det_blur));
    return det_blur;
}

template <typename T>
inline void add_blur_vjp(
    const T eps2d,
