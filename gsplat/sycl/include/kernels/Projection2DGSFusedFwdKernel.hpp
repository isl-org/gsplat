...
template <typename T> inline float sum(vec3<T> a) { return a.x + a.y + a.z; }

template <typename T> struct Projection2DGSFusedFwdKernel {
    const uint32_t m_B;
    const uint32_t m_C;
    const uint32_t m_N;
    ...
    Projection2DGSFusedFwdKernel(
        ...
        : m_B(B), m_C(C), m_N(N), m_means(means), m_quats(quats),
          m_scales(scales), m_viewmats(viewmats), m_Ks(Ks),
          m_image_width(image_width), m_image_height(image_height),
          m_near_plane(near_plane), m_far_plane(far_plane),
          m_radius_clip(radius_clip), m_radii(radii), m_means2d(means2d),
          m_depths(depths), m_ray_transforms(ray_transforms),
          m_normals(normals) {}
    ...
    void operator()(sycl::nd_item<1> work_item) const {
        ...
        mat3<T> rot_mat =
            quat_to_rotmat(vec4<T>(quats[0], quats[1], quats[2], quats[3]));
        ...
        mat3<T> world_2_pix = mat3<T>(
            Ks[0], T(0.0), Ks[2], T(0.0), Ks[4], Ks[5], T(0.0), T(0.0), T(1.0)
        );
        ...
        // Compute AABB
        const vec3<T> M0 =
            vec3<T>(M[0][0], M[0][1], M[0][2]); // first row of KWH
        const vec3<T> M1 =
            vec3<T>(M[1][0], M[1][1], M[1][2]); // second row of KWH
        const vec3<T> M2 =
            vec3<T>(M[2][0], M[2][1], M[2][2]); // third row of KWH
        ...
        const T radius_x =
            sycl::ceil(T(3.33) * sycl::sqrt(sycl::max(T(1e-4), half_extend.x)));
        const T radius_y =
            sycl::ceil(T(3.33) * sycl::sqrt(sycl::max(T(1e-4), half_extend.y)));
        ...
        if (mean2d.x + radius_x <= T(0) ||
            mean2d.x - radius_x >= m_image_width ||
            mean2d.y + radius_y <= T(0) ||
        ...
        // vec3<T> normal = vec3<T>(RS_camera[2][0], RS_camera[2][1],
        // RS_camera[2][2]);
