...
struct RasterizeToPixelsFwdKernel {
    ...
    const sycl::vec<S, 2>
        *m_means2d;          // [C, N, 2] or [nnz, 2] // <<< TYPE CHANGED
    ...
    RasterizeToPixelsFwdKernel(
        ...
        const sycl::vec<S, 2>
            *means2d,          // [C, N, 2] or [nnz, 2] // <<< TYPE CHANGED
        ...
        : m_C(C), m_N(N), m_n_isects(n_isects), m_packed(packed),
          ...
          m_slm_flatten_ids(slm_flatten_ids), m_slm_means2d(slm_means2d),
          m_slm_opacities(slm_opacities), m_slm_conics(slm_conics),
          m_slm_colors(slm_colors) {}
    ...
    void operator()(sycl::nd_item<3> work_item) const {
        ...
        // If a mask exists and the tile is marked false, output background
        // color immediately.
        ...
        const size_t threadRank =
            localId_y * groupWidth +
            localId_x; // given that range in 0th dimension is 1

        // Compute pixel coordinates: each work-item covers one pixel inside the
        // tile.
        ...
        for (uint32_t b = 0; b < numBatches; b++) {
            ...
            if (idx < range_end && threadRank < CHUNK_SIZE) {
                ...
                if constexpr (CONCAT_DATA) {
                    ...
                    if constexpr (COLOR_DIM == 3) {
                        ...
                        const S *data =
                            m_concatenated_data + g * m_concat_stride;
                        auto temp =
                        ...
                        if constexpr (BufferType<S, COLOR_DIM>::isVec &&
                            ...
                            auto temp2 =
                                *(reinterpret_cast<const sycl::vec<S, 2> *>(
                                    data + 8
                                ));
                            m_slm_means2d[threadRank] = {temp1[0], temp1[1]};
                            m_slm_conics[threadRank] = {
                                temp1[2], temp1[3], temp1[4]
                            };
                            m_slm_colors[threadRank] = {
                                temp1[5], temp1[6], temp1[7], temp2[0]
                            };
                            ...
                            m_slm_conics[threadRank] =
                                *(reinterpret_cast<const sycl::vec<S, 3> *>(
                                    data + 2
                                ));
                            m_slm_colors[threadRank] =
                                *(reinterpret_cast<
                    ...
                    if constexpr (BufferType<S, COLOR_DIM>::isVec &&
                                  COLOR_DIM <= 4) {
                        m_slm_colors[threadRank] =
                            *(reinterpret_cast<const BufferType_t<S, COLOR_DIM>
            ...
            for (int i = 0; i < endSize && (!done); i++) {
                ...
                S alpha =
                    sycl::min(static_cast<S>(0.999), opac * sycl::exp(-sigma));
        ...
        if (inside) {
            ...
            S *current_pixel_color_ptr_base =
                render_colors_ptr + pix_id * COLOR_DIM;
            auto *current_pixel_color_ptr =
                reinterpret_cast<BufferType_t<S, COLOR_DIM> *>(
                    current_pixel_color_ptr_base
                );

#pragma unroll
            for (uint32_t k = 0; k < COLOR_DIM; ++k) {
                current_pixel_color_ptr[0][k] =
                    pix_out[k] + T * backgroundColor[k];
            }
        }
    }
};
