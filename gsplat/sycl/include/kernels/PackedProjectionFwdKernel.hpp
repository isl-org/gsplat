...
template<typename T>
struct PackedProjectionFwdKernel {
    // Inputs
    const uint32_t m_B;
    const uint32_t m_C;
    const uint32_t m_N;
    ...
    PackedProjectionFwdKernel(
        ...
        const T* means, const T* covars, const T* quats, const T* scales, const T* opacities,
        const T* viewmats, const T* Ks,
        int32_t image_width, int32_t image_height,
        T eps2d, T near_plane, T far_plane, T radius_clip,
        CameraModelType camera_model,
        const int32_t* block_accum,
        // outputs
        int32_t* block_cnts, int32_t* indptr,
        int64_t* batch_ids, int64_t* camera_ids, int64_t* gaussian_ids,
        ...
        m_opacities(opacities), m_viewmats(viewmats), m_Ks(Ks),
        ...
        m_camera_model(camera_model), m_block_accum(block_accum),
        m_block_cnts(block_cnts), m_indptr(indptr),
        m_batch_ids(batch_ids), m_camera_ids(camera_ids), m_gaussian_ids(gaussian_ids),
        m_radii(radii), m_means2d(means2d), m_depths(depths), m_conics(conics), m_compensations(compensations)
    {}
    ...
    void operator()(sycl::nd_item<2> item) const {
        ...
        int32_t blocks_per_row = group_range[1]; // Get range of the 2nd dimension
        
        int32_t row_idx = group_id[0];           // Get group ID of the 1st dimension
        int32_t block_col_idx = group_id[1];     // Get group ID of the 2nd dimension
        ...
        if (valid) {
            ...
            R = mat3<T>(
                current_viewmats[0], current_viewmats[4], current_viewmats[8],
                current_viewmats[1], current_viewmats[5], current_viewmats[9],
                current_viewmats[2], current_viewmats[6], current_viewmats[10]
            );
            vec3<T> t(current_viewmats[3], current_viewmats[7], current_viewmats[11]);
        ...
        if (valid) {
            ...
            if (m_covars != nullptr) {
                ...
                covar = mat3<T>(
                    current_covars[0], current_covars[1], current_covars[2],
                    current_covars[1], current_covars[3], current_covars[4],
                    current_covars[2], current_covars[4], current_covars[5]
                );
            } else {
                const T* current_quats = m_quats + bid * m_N * 4 + gid * 4;
                const T* current_scales = m_scales + bid * m_N * 3 + gid * 3;
                quat_scale_to_covar_preci<T>(glm::make_vec4(current_quats), glm::make_vec3(current_scales), &covar, nullptr);
            }
            mat3<T> covar_c;
            ...
            switch (m_camera_model) {
                case CameraModelType::PINHOLE:
                    persp_proj<T>(mean_c, covar_c, current_Ks[0], current_Ks[4], current_Ks[2], current_Ks[5], m_image_width, m_image_height, covar2d, mean2d);
                    break;
                case CameraModelType::ORTHO:
                    ortho_proj<T>(mean_c, covar_c, current_Ks[0], current_Ks[4], current_Ks[2], current_Ks[5], m_image_width, m_image_height, covar2d, mean2d);
                    break;
                case CameraModelType::FISHEYE:
                    fisheye_proj<T>(mean_c, covar_c, current_Ks[0], current_Ks[4], current_Ks[2], current_Ks[5], m_image_width, m_image_height, covar2d, mean2d);
                    break;
            }
        ...
        if (m_block_cnts != nullptr) {
            ...
            if (any_valid) {
                ...
                int32_t aggregate = sycl::reduce_over_group(group, thread_data, sycl::plus<>());
                if (local_id == 0) {
            ...
            bool any_valid = sycl::any_of_group(group, valid);
            if (any_valid) {
                // Perform an exclusive scan to find the local offset for this thread.
                int32_t local_offset = sycl::exclusive_scan_over_group(group, thread_data, sycl::plus<>());
            ...
            if (local_id == 0 && block_col_idx == 0) {
                if (row_idx == 0) {
                    m_indptr[0] = 0;
                    // The final count is written by the host after a scan over block_accum.
                    // m_indptr[m_B * m_C] = m_block_accum[m_B * m_C * blocks_per_row - 1];
                } else {
