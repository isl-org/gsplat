...
template <typename T> inline T sum(vec3<T> a) { return a.x + a.y + a.z; }
...
inline void compute_ray_transforms_aabb_vjp(
    ...
    if (v_means2d[0] != 0 || v_means2d[1] != 0) {
        ...
        const T dpx_dT30 =
            ray_transforms[0] * f + T(2) * dpx_dd * ray_transforms[6];
        const T dpx_dT31 =
            ray_transforms[1] * f + T(2) * dpx_dd * ray_transforms[7];
        const T dpx_dT32 =
            -ray_transforms[2] * f - T(2) * dpx_dd * ray_transforms[8];
        const T dpy_dd = -f * f *
                         (ray_transforms[3] * ray_transforms[6] +
                          ray_transforms[4] * ray_transforms[7] -
                          ray_transforms[5] * ray_transforms[8]);
        const T dpy_dT30 =
            ray_transforms[3] * f + T(2) * dpy_dd * ray_transforms[6];
        const T dpy_dT31 =
            ray_transforms[4] * f + T(2) * dpy_dd * ray_transforms[7];
        const T dpy_dT32 =
            -ray_transforms[5] * f - T(2) * dpy_dd * ray_transforms[8];
    ...
    mat3<T> RS = quat_to_rotmat(quat) * mat3<T>(
                                            scale[0],
                                            T(0.0),
                                            T(0.0),
                                            T(0.0),
...
template <typename T> struct Projection2DGSFusedBwdKernel {
    // fwd inputs
    const uint32_t m_B;
    const uint32_t m_C;
    const uint32_t m_N;
    ...
    Projection2DGSFusedBwdKernel(
        ...
        : m_B(B), m_C(C), m_N(N), m_means(means), m_quats(quats),
          ...
          m_v_ray_transforms(v_ray_transforms), m_v_means(v_means),
    ...
    void operator()(sycl::nd_item<1> work_item) const {
        ...
        vec4<T> quat =
            vec4<T>(quats_ptr[0], quats_ptr[1], quats_ptr[2], quats_ptr[3]);
