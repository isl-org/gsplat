...
template <typename T> struct PackedProjectionBwdKernel {
    // fwd inputs
    const uint32_t m_B;
    const uint32_t m_C;
    const uint32_t m_N;
    ...
    PackedProjectionBwdKernel(
        ...
        : m_B(B), m_C(C), m_N(N), m_nnz(nnz), m_means(means), m_covars(covars),
          m_quats(quats), m_scales(scales), m_viewmats(viewmats), m_Ks(Ks),
          m_image_width(image_width), m_image_height(image_height),
          m_eps2d(eps2d), m_camera_model(camera_model), m_batch_ids(batch_ids),
          ...
          m_v_compensations(v_compensations), m_sparse_grad(sparse_grad),
          m_v_means(v_means), m_v_covars(v_covars), m_v_quats(v_quats),
          m_v_scales(v_scales), m_v_viewmats(v_viewmats) {}
    ...
    void operator()(sycl::nd_item<1> item) const {
        ...
        mat2<T> v_covar2d(0.f);
        {
            ...
            mat2<T> v_covar2d_inv = mat2<T>(
                v_conics[0], v_conics[1] * 0.5f, v_conics[1] * 0.5f, v_conics[2]
            );
        ...
        if (m_sparse_grad) {
            ...
            if (m_v_means != nullptr) {
                ...
                v_means_out[0] = v_mean.x;
                v_means_out[1] = v_mean.y;
                v_means_out[2] = v_mean.z;
            }
            if (m_v_covars != nullptr) {
                ...
                v_quats_out[0] = v_quat.x;
                v_quats_out[1] = v_quat.y;
                v_quats_out[2] = v_quat.z;
                v_quats_out[3] = v_quat.w;
                v_scales_out[0] = v_scale.x;
                v_scales_out[1] = v_scale.y;
                v_scales_out[2] = v_scale.z;
            }
        } else {
