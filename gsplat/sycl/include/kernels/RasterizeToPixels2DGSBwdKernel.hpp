#ifndef RASTERIZE_TO_PIXELS_2DGS_BWD_KERNEL_HPP
#define RASTERIZE_TO_PIXELS_2DGS_BWD_KERNEL_HPP
...
struct RasterizeToPixels2DGSBwdKernel {
    ...
    // Gradient outputs
    sycl::vec<float, 2>* m_v_means2d_abs;     // Gradients of means2d (absolute, can be null)
    ...
    RasterizeToPixels2DGSBwdKernel(
        ...
        m_v_means2d_abs(v_means2d_abs), m_v_means2d(v_means2d), m_v_ray_transforms(v_ray_transforms),
        m_v_colors(v_colors), m_v_opacities(v_opacities), m_v_normals(v_normals), m_v_densify(v_densify),
    ...
    void operator()(sycl::nd_item<3> item) const {
        ...
        const float* v_render_colors_ptr = m_v_render_colors + image_id * m_image_height * m_image_width * COLOR_DIM;
        const float* v_render_alphas_ptr = m_v_render_alphas + image_id * m_image_height * m_image_width;
        const float* v_render_normals_ptr = m_v_render_normals + image_id * m_image_height * m_image_width * 3;
        const float* v_render_distort_ptr = nullptr;
        if (m_v_render_distort != nullptr) {
            v_render_distort_ptr = m_v_render_distort + image_id * m_image_height * m_image_width;
        }
        const float* v_render_median_ptr = m_v_render_median + image_id * m_image_height * m_image_width;
        ...
        const int32_t pix_id = static_cast<int32_t>( sycl::min(static_cast<uint32_t>(i * m_image_width + j), 
        ...
        int32_t range_end;
        if ((image_id == m_I - 1) && (tile_id == static_cast<int32_t>(m_tile_width * m_tile_height - 1))) {
        ...
        if (inside) {
            if constexpr(BufferType<float, COLOR_DIM>::isVec && COLOR_DIM <= 4) {
                v_render_c = *reinterpret_cast<const BufferType_t<float, COLOR_DIM>*>(v_render_colors_ptr + pix_id * COLOR_DIM);
            } else {
        ...
        float accum_d_buffer = 0.0f, accum_w_buffer = 0.0f, distort_buffer = 0.0f;
        if (v_render_distort_ptr != nullptr && inside) {
            v_distort = v_render_distort_ptr[pix_id];
            accum_d_buffer = render_colors_ptr[pix_id * COLOR_DIM + COLOR_DIM - 1];
        ...
        int32_t warp_bin_final = sycl::reduce_over_group(
            item.get_sub_group(), bin_final, 
            sycl::maximum<int32_t>()
        );
        ...
        for (int32_t b = 0; b < num_batches; ++b) {
            ...
            int32_t batch_size = sycl::min<int32_t>(m_chunk_size, batch_end + 1 - range_start);
            ...
            if (idx >= range_start && tr < m_chunk_size) {
                ...
                // Load colors
                if constexpr(BufferType<float, COLOR_DIM>::isVec && COLOR_DIM <= 4) {
                    m_slm_rgbs[tr] = *reinterpret_cast<const BufferType_t<float, COLOR_DIM>*>(m_colors + g * COLOR_DIM);
                } else {
                    for (uint32_t k = 0; k < COLOR_DIM; ++k) {
            ...
            for (int32_t t = sycl::max(0, batch_end - warp_bin_final); t < batch_size; ++t) {
                bool valid = inside;
                ...
                float gauss_weight_3d = 0.0f, gauss_weight_2d = 0.0f, gauss_weight = 0.0f;
                sycl::vec<float, 2> s{0.0f, 0.0f}, d{0.0f, 0.0f};
                sycl::vec<float, 3> h_u{0.0f, 0.0f, 0.0f}, h_v{0.0f, 0.0f, 0.0f};
                sycl::vec<float, 3> ray_cross{0.0f, 0.0f, 0.0f}, w_M{0.0f, 0.0f, 0.0f};
                ...
                if (valid) {
                    ...
                    if (ray_cross[2] == 0.0f) {
                        ...
                        // Project to UV space
                        s = sycl::vec<float, 2>(ray_cross[0] / ray_cross[2], ray_cross[1] / ray_cross[2]);
                        
                        // Calculate 3D gaussian weight
                        gauss_weight_3d = s[0] * s[0] + s[1] * s[1];
                        
                        // Calculate 2D projected gaussian weight
                        d = sycl::vec<float, 2>(xy_opac[0] - px, xy_opac[1] - py);
                        gauss_weight_2d = FILTER_INV_SQUARE_2DGS * (d[0] * d[0] + d[1] * d[1]);
                        
                        // Use minimum of 3D and 2D weights
                        gauss_weight = sycl::min(gauss_weight_3d, gauss_weight_2d);
                ...
                bool any_valid = sycl::any_of_group(item.get_sub_group(), valid);
                if (!any_valid) {
                    continue;
                }
                ...
                if (valid) {
                    ...
                    float v_alpha = 0.0f;
                    for (uint32_t k = 0; k < COLOR_DIM; ++k) {
                        v_alpha += (m_slm_rgbs[t][k] * T - buffer[k] * ra) * v_render_c[k];
                    }
                    ...
                    for (uint32_t k = 0; k < 3; ++k) {
                        v_alpha += (m_slm_normals[t][k] * T - buffer_normals[k] * ra) * v_render_n[k];
                    }
                    ...
                    if (v_render_distort_ptr != nullptr) {
                        ...
                        float dl_dw = 2.0f * (2.0f * (depth * accum_w_buffer - accum_d_buffer) + (accum_d - depth * accum_w));
                        v_alpha += (dl_dw * T - distort_buffer * ra) * v_distort;
                        accum_d_buffer -= fac * depth;
                        accum_w_buffer -= fac;
                        distort_buffer += dl_dw * fac;
                        v_rgb_local[COLOR_DIM - 1] += 2.0f * fac * (2.0f - 2.0f * T - accum_w + fac) * v_distort;
                    }
                    ...
                    if (opac * vis <= 0.999f) {
                        ...
                        // Case 1: Ray-primitive intersection used in forward pass
                        if (gauss_weight_3d <= gauss_weight_2d) {
                            ...
                            sycl::vec<float, 3> v_ray_cross(
                                v_sx_pz, v_sy_pz, -(v_sx_pz * s[0] + v_sy_pz * s[1])
                            );
                            
                            // Calculate cross products for gradient computation
                            sycl::vec<float, 3> v_h_u = sycl::cross(h_v, v_ray_cross);
                            sycl::vec<float, 3> v_h_v = sycl::cross(v_ray_cross, h_u);
                            
                            // Compute gradients for transformation matrices
                            v_u_M_local = sycl::vec<float, 3>(-v_h_u[0], -v_h_u[1], -v_h_u[2]);
                            v_v_M_local = sycl::vec<float, 3>(-v_h_v[0], -v_h_v[1], -v_h_v[2]);
                            v_w_M_local = sycl::vec<float, 3>(
                                px * v_h_u[0] + py * v_h_v[0] + v_depth * v_z_w_M[0],
                                px * v_h_u[1] + py * v_h_v[1] + v_depth * v_z_w_M[1],
                                px * v_h_u[2] + py * v_h_v[2] + v_depth * v_z_w_M[2]
                            );
                            
                        // Case 2: 2D projected gaussian used in forward pass
                        } else {
                            float v_G_ddelx = -vis * FILTER_INV_SQUARE_2DGS * d[0];
                            float v_G_ddely = -vis * FILTER_INV_SQUARE_2DGS * d[1];
                            v_xy_local = sycl::vec<float, 2>(v_G * v_G_ddelx, v_G * v_G_ddely);
                            
                            if (m_v_means2d_abs != nullptr) {
                                v_xy_abs_local = sycl::vec<float, 2>(
                                    sycl::fabs(v_xy_local[0]), sycl::fabs(v_xy_local[1])
                                );
                            }
                        }
                ...
                if constexpr(BufferType<float, COLOR_DIM>::isVec && COLOR_DIM <= 4) {
                    v_rgb_local = sycl::reduce_over_group(sub_group, v_rgb_local, sycl::plus<BufferType_t<float, COLOR_DIM>>());
                } else {
                    for (uint32_t k = 0; k < COLOR_DIM; ++k) {
                        v_rgb_local[k] = sycl::reduce_over_group(sub_group, v_rgb_local[k], sycl::plus<float>());
                    }
                }
                
                // Reduce other gradients
                v_normal_local = sycl::reduce_over_group(sub_group, v_normal_local, sycl::plus<sycl::vec<float, 3>>());
                v_u_M_local = sycl::reduce_over_group(sub_group, v_u_M_local, sycl::plus<sycl::vec<float, 3>>());
                v_v_M_local = sycl::reduce_over_group(sub_group, v_v_M_local, sycl::plus<sycl::vec<float, 3>>());
                v_w_M_local = sycl::reduce_over_group(sub_group, v_w_M_local, sycl::plus<sycl::vec<float, 3>>());
                v_xy_local = sycl::reduce_over_group(sub_group, v_xy_local, sycl::plus<sycl::vec<float, 2>>());
                v_opacity_local = sycl::reduce_over_group(sub_group, v_opacity_local, sycl::plus<float>());
                
                if (m_v_means2d_abs != nullptr) {
                    v_xy_abs_local = sycl::reduce_over_group(sub_group, v_xy_abs_local, sycl::plus<sycl::vec<float, 2>>());
                }
                ...
                if (sub_group.get_local_id() == 0) {
                    ...
                    for (uint32_t k = 0; k < COLOR_DIM; ++k) {
                        gpuAtomicAddGlobal(m_v_colors[g * COLOR_DIM + k], v_rgb_local[k]);
                    }
                    
                    // Update normal gradients
                    for (uint32_t k = 0; k < 3; ++k) {
                        gpuAtomicAddGlobal(m_v_normals[g * 3 + k], v_normal_local[k]);
                    }
                    
                    // Update ray transform gradients
                    gpuAtomicAddGlobal(m_v_ray_transforms[g * 9], v_u_M_local[0]);
                    gpuAtomicAddGlobal(m_v_ray_transforms[g * 9 + 1], v_u_M_local[1]);
                    gpuAtomicAddGlobal(m_v_ray_transforms[g * 9 + 2], v_u_M_local[2]);
                    gpuAtomicAddGlobal(m_v_ray_transforms[g * 9 + 3], v_v_M_local[0]);
                    gpuAtomicAddGlobal(m_v_ray_transforms[g * 9 + 4], v_v_M_local[1]);
                    gpuAtomicAddGlobal(m_v_ray_transforms[g * 9 + 5], v_v_M_local[2]);
                    gpuAtomicAddGlobal(m_v_ray_transforms[g * 9 + 6], v_w_M_local[0]);
                    gpuAtomicAddGlobal(m_v_ray_transforms[g * 9 + 7], v_w_M_local[1]);
                    gpuAtomicAddGlobal(m_v_ray_transforms[g * 9 + 8], v_w_M_local[2]);
                    ...
                    if (m_v_means2d_abs != nullptr) {
                        gpuAtomicAddGlobal(m_v_means2d_abs[g].x(), v_xy_abs_local[0]);
                        gpuAtomicAddGlobal(m_v_means2d_abs[g].y(), v_xy_abs_local[1]);
                    }
                ...
                if (valid) {
                    ...
                    m_v_densify[g * 2 + 1] = m_v_ray_transforms[g * 9 + 5] * depth;
                }
            }
        }
    }
...
#endif // RASTERIZE_TO_PIXELS_2DGS_BWD_KERNEL_HPP
