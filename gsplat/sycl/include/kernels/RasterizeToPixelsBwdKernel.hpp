...
struct RasterizeToPixelsBwdKernel
{
	...
	RasterizeToPixelsBwdKernel(
		...
		m_tile_offsets(tile_offsets), m_flatten_ids(flatten_ids), m_render_alphas(render_alphas), 
		m_last_ids(last_ids), m_v_render_colors(v_render_colors), m_v_render_alphas(v_render_alphas), 
		m_v_means2d_abs(v_means2d_abs), m_v_means2d(v_means2d), m_v_conics(v_conics), m_v_colors(v_colors), 
		m_v_opacities(v_opacities), m_slm_flatten_ids(slm_flatten_ids), m_slm_means2d(slm_means2d), 
		m_slm_opacities(slm_opacities), m_slm_conics(slm_conics), m_slm_colors(slm_colors)
	{
	}
	...
	{
		// Compute camera and tile indices (each work-group corresponds to a tile)
		...
		const int32_t pix_id = sycl::min(static_cast<int32_t>(i * m_image_width + j),
		...
		const S *render_alphas_ptr = m_render_alphas + camera_id * m_image_height * m_image_width;
		const int32_t *last_ids_ptr = m_last_ids + camera_id * m_image_height * m_image_width;
		const S *v_render_colors_ptr = m_v_render_colors + camera_id * m_image_height * m_image_width * COLOR_DIM;
		const S *v_render_alphas_ptr = m_v_render_alphas + camera_id * m_image_height * m_image_width;
		...
		const size_t threadRank = work_item.get_local_linear_id(); // given that range in 0th dimension is 1
		...
		for(int32_t b = numBatches-1; b >= 0; b--) {
			...
			if (loadIdx < range_end && threadRank < CHUNK_SIZE) {
				...
				if constexpr( CONCAT_DATA) {
					...
					if constexpr(COLOR_DIM == 3) {
						...
						auto temp16 = temp.template convert<sycl::half, sycl::rounding_mode::automatic>();
						...
						auto xy = *(reinterpret_cast<const sycl::vec<S,2>*>(data) );
						m_slm_means2d[threadRank] = xy.template convert<sycl::half, sycl::rounding_mode::automatic>();

						auto conic = *(reinterpret_cast<const sycl::vec<S,3>*>(data+2) );
						m_slm_conics[threadRank] = conic.template convert<sycl::half, sycl::rounding_mode::automatic>();

						if constexpr(BufferType<S, COLOR_DIM>::isVec && COLOR_DIM <= 4){
							auto color = *( reinterpret_cast<const BufferType_t<S, COLOR_DIM>*>(data + 2 + 3) );
							m_slm_colors[threadRank] = color.template convert<sycl::half, sycl::rounding_mode::automatic>();;
						}
					}
					m_slm_opacities[threadRank] = static_cast<sycl::half>(*(data + 2 + 3 + COLOR_DIM));
				
				} else {
					m_slm_means2d[threadRank] = m_means2d[g].template convert<sycl::half, sycl::rounding_mode::automatic>();

					m_slm_opacities[threadRank] = static_cast<sycl::half>(m_opacities[g]);
					auto temp = *( reinterpret_cast<const sycl::vec<S, 3>*>(m_conics + g) );
					
					m_slm_conics[threadRank] = temp.template convert<sycl::half, sycl::rounding_mode::automatic>();

					if constexpr(BufferType<S, COLOR_DIM>::isVec && COLOR_DIM <= 4){
						auto temp2 = *( reinterpret_cast<const BufferType_t<S, COLOR_DIM>*>(m_colors + g * COLOR_DIM) );
						m_slm_colors[threadRank] = temp2.template convert<sycl::half, sycl::rounding_mode::automatic>();
					}
				}
			}
			...
			for(int32_t idx = numel-1; idx >= 0; idx-- ) {
				// Only process gaussians that actually contributed in the forward pass.
				...
				sycl::vec<S, 2> xy = m_slm_means2d[idx].template convert<S, sycl::rounding_mode::automatic>();
				const S opac = static_cast<S>(m_slm_opacities[idx]);
				auto conic = m_slm_conics[idx].convert<S, sycl::rounding_mode::automatic>();
				
				BufferType_t<S, COLOR_DIM> rgb;
				if constexpr(BufferType<S, COLOR_DIM>::isVec && COLOR_DIM <= 4){
					rgb = m_slm_colors[idx].template convert<S, sycl::rounding_mode::automatic>();	
				} else {
					if constexpr(CONCAT_DATA) {
						readToBuffer(rgb, m_concatenated_data + g*m_concat_stride + 2 + 3);
					} else {
				...
				S sigma = static_cast<S>(0.5) * (conic.x() * delta.x() * delta.x() + conic.z() * delta.y() * delta.y()) + conic.y() * delta.x() * delta.y();
				...
				if (toProcess) {
					...
					const S ra = static_cast<S>(1.0) / (static_cast<S>(1.0) - alpha);
					T *= ra;
					const S fac = alpha * T;
					...
					for (uint32_t k = 0; k < COLOR_DIM; ++k)
					{
						v_alpha += (rgb[k] * T - buffer[k] * ra) * v_render_c[k];
					}
					...
					if (opac * vis <= static_cast<S>(0.999))
					{
						...
						v_conic_local[0] = static_cast<S>(0.5) * v_sigma * delta.x() * delta.x();
						v_conic_local[1] = v_sigma * delta.x() * delta.y();
						v_conic_local[2] = static_cast<S>(0.5) * v_sigma * delta.y() * delta.y();
						v_xy_local[0] = v_sigma * (conic.x() * delta.x() + conic.y() * delta.y());
						v_xy_local[1] = v_sigma * (conic.y() * delta.x() + conic.z() * delta.y());
				...
				if constexpr( BufferType<S,COLOR_DIM>::isVec ) {
					local_color = sycl::reduce_over_group( work_item.get_group(), v_rgb_local, sycl::plus<BufferType_t<S, COLOR_DIM>>());
				} else {
					for (uint32_t k = 0; k < COLOR_DIM; ++k) {
						local_color[k] = sycl::reduce_over_group( work_item.get_group(), v_rgb_local[k], sycl::plus<S>());
					}
				}
				
				S local_opacity = sycl::reduce_over_group( work_item.get_group(), v_opacity_local, sycl::plus<S>());
				auto local_conic = sycl::reduce_over_group( work_item.get_group(), v_conic_local, sycl::plus<sycl::vec<S, 3>>());
				auto local_mean = sycl::reduce_over_group( work_item.get_group(), v_xy_local, sycl::plus<sycl::vec<S, 2>>());
				...
				if(threadRank == idx) {
					for (uint32_t k = 0; k < COLOR_DIM; ++k) {
						gpuAtomicAddGlobal(m_v_colors[g * COLOR_DIM + k], local_color[k]);
					}
					...
					if (m_v_means2d_abs != nullptr) {
						gpuAtomicAddGlobal(m_v_means2d_abs[g].x(),local_mean_abs[0]);
                    	gpuAtomicAddGlobal(m_v_means2d_abs[g].y(),local_mean_abs[1]);
					}
				}
			}
		}
