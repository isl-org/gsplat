...
template <typename T> struct WorldToCamBwdKernel {
    const uint32_t m_C;
    const uint32_t m_N;
    const T *m_means;      // [N, 3]
    ...
    WorldToCamBwdKernel(
        ...
        : m_C(C), m_N(N), m_means(means), m_covars(covars),
          m_viewmats(viewmats), m_v_means_c(v_means_c),
          m_v_covars_c(v_covars_c), m_v_means(v_means), m_v_covars(v_covars),
    ...
    void operator()(sycl::nd_item<1> work_item) const {
        ...
        if (m_v_covars_c != nullptr) {
            const mat3<T> v_covar_c_t =
                glm::make_mat3(m_v_covars_c + (idx * 9));
