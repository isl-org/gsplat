...
template <uint32_t COLOR_DIM> struct RasterizeToPixels2DGSFwdKernel {
    ...
    RasterizeToPixels2DGSFwdKernel(
        ...
        : m_I(I), m_N(N), m_n_isects(n_isects), m_packed(packed),
          ...
          m_render_alphas(render_alphas), m_render_normals(render_normals),
          m_render_distort(render_distort), m_render_median(render_median),
    ...
    void operator()(sycl::nd_item<3> item) const {
        ...
        float *render_colors_ptr = m_render_colors + image_id * m_image_height *
                                                         m_image_width *
                                                         COLOR_DIM;
        float *render_alphas_ptr =
        ...
        float *render_normals_ptr =
            m_render_normals + image_id * m_image_height * m_image_width * 3;
        float *render_distort_ptr =
            m_render_distort + image_id * m_image_height * m_image_width;
        float *render_median_ptr =
            m_render_median + image_id * m_image_height * m_image_width;
        ...
        int32_t range_end =
            (image_id == m_I - 1) &&
                    (tile_id ==
                     static_cast<int32_t>(m_tile_width * m_tile_height - 1))
                ? m_n_isects
        ...
        for (uint32_t b = 0; b < num_batches; ++b) {
            ...
            // Manual check for all threads done (instead of CUDA's
            // __syncthreads_count) In SYCL, we have to use barrier
            // synchronization and local variables for this

            // Process gaussians in the current batch
            uint32_t batch_size =
                sycl::min(m_chunk_size, range_end - batch_start);
            for (uint32_t t = 0; t < batch_size && !done; ++t) {
                ...
                sycl::vec<float, 2> s(
                    ray_cross[0] / ray_cross[2], ray_cross[1] / ray_cross[2]
                );
                ...
                sycl::vec<float, 2> d(xy_opac[0] - px, xy_opac[1] - py);
                // gauss_weight_2d = FILTER_INV_SQUARE_2DGS * (d.x * d.x + d.y *
                // d.y)
                float gauss_weight_2d =
                    FILTER_INV_SQUARE_2DGS * (d[0] * d[0] + d[1] * d[1]);
                ...
                if constexpr (BufferType<float, COLOR_DIM>::isVec &&
                              COLOR_DIM <= 4) {
                    const auto *c_ptr = reinterpret_cast<
        ...
        if (inside) {
            ...
            if (backgrounds_ptr == nullptr) {
                ...
                if constexpr (BufferType<float, COLOR_DIM>::isVec &&
                              COLOR_DIM <= 4) {
                    *reinterpret_cast<BufferType_t<float, COLOR_DIM> *>(
